  ...
  try {
    ...
    if (request.method !== 'GET') {
      ...
      for (const [key,value] of query) {
        if (!['select','order','active','accepted','recorded_at','id','limit','offset','client_id','site_id'].includes(key) || value.length > 1000) throw new HttpError(400,'Unsupported query');
      }
